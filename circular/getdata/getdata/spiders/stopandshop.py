...
class StopandshopSpider(CrawlSpider):
    '''
    ...
    def __init__(self, zip_code='', *args, **kwargs):
        super(StopandshopSpider, self).__init__(*args, **kwargs)
        self.start_urls = ['http://www.shoplocal.com/',
                           'http://www.shoplocal.com/changelocation?citystatezip=%s' % zip_code]

    rules = (
        Rule(SgmlLinkExtractor(allow=r'stop-and-shop/weekly-ads-sales/'),
             callback='parse_main_page', follow=True),
    )

    def parse_main_page(self, response):
        '''
        parse_main_page parse the page http://www.shoplocal.com/ and
        return all links that include:
        ...
        for link in main_data:
            if 'stop-and-shop/weekly-ads-sales/' in \
            link and 'promotionpagenumbers' in link:
                link = self.start_urls[0] + link
                yield Request(link, callback=self.parse_deals_links)

    def parse_deals_links(self, response):
        '''
        parse_deals_links parse all links returned by parse_main_page
        and extract likns that under the <div class="sl_deal_tile">
        the extracted links will be procesed through parse_prduct.
        '''
        sel = Selector(response)
        location = StoreLocationItem()
        hold = sel.xpath('//div[@class="sl_retailer_info fl"]')
        city_state = hold.xpath('//span[@class="sl_retailer_address2"\
                                ]/text()').extract()[0].strip().split(',')
        ...
        location['zip_code'] = get_zipcode(location)

        deal_data = sel.xpath('//div[@class="sl_deal_tile"]//a/@href')\
        .extract()
    ...
    def parse_product(self, response):
        ...
        data['discount_price'] = product.xpath(
            '//div[@class="sl_detail_dealdesc"]/text()'
            ).extract()[0].strip()
        ...
        [data['start_date'], data['end_date']] = deal_valid(
            product.xpath(
                '//div[@class="sl_detail_dates"]/text()'
                ).extract()[0].strip())
